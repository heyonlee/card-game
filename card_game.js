...
while(currentIndex != 0){ 
  ...
  shuffledDeck[randomIndex] = temporaryValue ;   // from the last card, we will pick one card and also we will pick one card randomly and we will switch it *46
  currentIndex--;
...
const compare = (current,next)=>{
 ...
 const guess = (current, next) => { 
   console.log( `The current card is ${current.suit} ${current.rank}`)

   let input =getInput('higher or lower (h/l)')
   if(input === 'h'){ 
   ...
   else{ console.log ("You need to choose h or l. Otherwise no points for this round ")
      return false;
 }
 }   
...
// 2. declare a variable called deck (it's okay to reuse -- remember scope!) 
      //that takes the result of the shuffle function. 
...
// 9. Close the conditional statement and assign nextCard to currentCard. 
     //You may have to write this as the type of variable that's always global...
